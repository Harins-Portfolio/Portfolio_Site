  ...
  async createProject(data) {
    ...
    if (error) {
      if (error.message && error.message.includes('password_hash')) {
        throw new Error('The "password_hash" column is missing from your Supabase "projects" table. Please add it: ALTER TABLE projects ADD COLUMN password_hash TEXT;');
      }
      handleError(error);
    }
    return project;
